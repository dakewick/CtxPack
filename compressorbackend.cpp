...
static const int PROCESS_TIMEOUT_MS = 300000;                 // 进程超时(300秒)
static const char* RADAR_SCRIPT = "code_radar.py";           // 代码雷达脚本名
static const int RADAR_TIMEOUT_MS = 10000;                    // 雷达搜索超时(10秒)
    ...
    connect(m_process, QOverload<int, QProcess::ExitStatus>::of(&QProcess::finished),
            this, [this](int exitCode, QProcess::ExitStatus exitStatus) {
                ...
                // ---------- 失败路径 ----------
                    ...
                    // 💡 修复：使用 multi-arg 一次性替换，避免产生多个临时 QString
                    content = QString("❌ 压缩脚本%1 (退出码: %2)\n\n"
                                      ...
                                      "3. 查看控制台错误日志")
                                  .arg(errorType,
                                       QString::number(exitCode), // int 需显式转换为 QString
                                       m_currentProjectPath,
                                       m_lastError.isEmpty() ? "无" : m_lastError);
                }
...
QString CompressorBackend::resolvePython() const {
    QProcess test;
    test.start("python3", QStringList() << "--version");
...
// ============================================
// 代码雷达：搜索指定字符串在项目中的出现位置
// ============================================
QVariantMap CompressorBackend::searchCodeRadar(const QString &projectDir, const QString &target) {
    QVariantMap result;
    ...
    // 🧹 优化 1：清理路径格式，将所有反斜杠转换为标准的 Unix 斜杠，防止 Windows 路径传参断层
    QString localPath = QUrl(projectDir).toLocalFile();
    if (localPath.isEmpty()) {
    ...
    if (!dirInfo.exists() || !dirInfo.isDir()) {
        result["status"] = "error";
        result["message"] = "无效的项目路径: " + localPath;
        return result;
    }

    QString scriptPath = QCoreApplication::applicationDirPath() + "/" + RADAR_SCRIPT;
    if (!QFileInfo::exists(scriptPath)) {
        result["status"] = "error";
        result["message"] = "找不到雷达脚本: " + scriptPath;
        return result;
    }

    // 🧹 优化 2：动态分配 QProcess，配合 QEventLoop 实现局部异步，彻底解决 UI 假死（白屏）问题
    QProcess *radar = new QProcess();
    radar->setProcessChannelMode(QProcess::SeparateChannels);
    ...
    // 绑定进程结束信号，退出事件循环
    connect(radar, QOverload<int, QProcess::ExitStatus>::of(&QProcess::finished), &loop, &QEventLoop::quit);
    ...
    radar->start(resolvePython(), QStringList()
                                      << "-X" << "utf8"
                                      << scriptPath
                                      << localPath
                                      << target.trimmed());

    if (!radar->waitForStarted(3000)) {
        result["status"] = "error";
        result["message"] = "无法启动 Python 进程，请确认 python 已加入 PATH";
        radar->deleteLater();
        return result;
    }

    // 🚀 启动超时定时器，并阻塞在此处，但 QEventLoop 会保持前端 UI 动画和点击事件极其流畅！
    timeoutTimer.start(RADAR_TIMEOUT_MS);
    loop.exec();

    // 处理超时结果
    if (isTimeout) {
        result["status"] = "error";
        result["message"] = "雷达搜索超时 (10s)，项目可能过大或脚本陷入死循环";
        radar->deleteLater();
        return result;
    }
    ...
    // 验证退出状态
    if (radar->exitCode() != 0 || radar->exitStatus() != QProcess::NormalExit) {
        QString errOutput = QString::fromUtf8(radar->readAllStandardError()).trimmed();
        result["status"] = "error";
        result["message"] = errOutput.isEmpty() ? "脚本异常退出，退出码: " + QString::number(radar->exitCode()) : errOutput;
        radar->deleteLater();
        return result;
    }

    QByteArray output = radar->readAllStandardOutput();
    radar->deleteLater(); // 清理进程对象
    ...
    if (parseError.error != QJsonParseError::NoError) {
        result["status"] = "error";
        result["message"] = "JSON 解析失败: " + parseError.errorString()
                            + "\n原始输出: " + QString::fromUtf8(output.left(500));
        return result;
    }

    if (!doc.isObject()) {
        result["status"] = "error";
        result["message"] = "脚本返回了非 JSON 对象格式的数据";
        return result;
    }
    ...
    if (status == "error") {
        result["status"] = "error";
        result["message"] = json["message"].toString("未知错误");
        return result;
    }

    result["status"] = "success";
    result["count"] = json["count"].toInt(0);
    result["target"] = json["target"].toString();

    QVariantList items;
    QJsonArray arr = json["results"].toArray();

    for (const QJsonValue &val : std::as_const(arr)) {
        QJsonObject obj = val.toObject();
        QVariantMap item;
        item["file"] = obj["file"].toString();
        item["line"] = obj["line"].toInt();
...
void CompressorBackend::cancelCompression() {
    if (m_isRunning) {
        qDebug() << "[Compressor] 用户请求取消压缩";

        if (m_process->state() != QProcess::NotRunning) {
            // 🧹 1. 先尝试优雅终止 (发送 SIGTERM)
            m_process->terminate();

            // 🧹 2. 给进程 1000ms 处理后事
            if (!m_process->waitForFinished(1000)) {
                qWarning() << "[Compressor] 脚本未能优雅退出，强制执行 Kill";
                m_process->kill(); // 只有当 terminate 失效时才暴力击杀
            }
        }

        // 🧹 3. 防止残留的临时文件导致下次读取失败
        QString outputFilePath = QDir(m_process->workingDirectory()).absoluteFilePath(OUTPUT_FILE);
        QFile::remove(outputFilePath);

        finishProcess("⏹️ 压缩已中止，并已清理后台进程", false);
    }
}